#!/usr/bin/env python3
# ruff: noqa: E402
"""Experiment 2: baseline (selfish) vs. inequity-averse agents on Harvest.

See run_experiment1_cleanup.py's docstring -- same comparison, same smoke-
test-by-default convention, applied to the tragedy-of-the-commons dilemma
instead of the public-goods one.
"""
import argparse
import json
import sys
...
def run_condition(name: str, use_inequity_reward: bool, args) -> dict:
    ...
    env = HarvestEnv(num_agents=args.num_agents, config=GridWorldConfig(episode_length=args.episode_length), rng=rng)
    ...
    if use_inequity_reward:
        ...
        # See run_experiment1_cleanup.py's comment / scaled_alpha_beta_range's
        # docstring for why the base range is amplification-corrected here.
...
def main():
    ...
    parser.add_argument("--out-dir", type=str, default="output/run_experiment2_harvest")
    ...
    try:
        ...
        ax.set_title("Harvest: baseline vs. inequity-averse agents")
